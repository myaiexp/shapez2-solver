// Unit tests for _getConnectedSingleLayer wrap-around — run with:
//   node tests/connectedSingleLayer.test.js
// Covers the FORWARD wrap-around path: the forward walk overruns the end of the
// layer and _getCorrectedIndex folds the index back to the start (e.g. quadrant
// 3 -> quadrant 0). The crystal suite only exercises the backward wrap, so this
// guards the positive-overflow branch of _getCorrectedIndex. A contrasting
...
import { _gravityConnected, _getConnectedSingleLayer } from '../shapeOperationsHelpers.js';
...
// four parts per layer. '-' = empty; under _gravityConnected any two non-empty,
...
check('forward wrap: quadrant 3 connects across the seam to 0 and 1',
    _getConnectedSingleLayer(layerOf('CuCu--Cu'), 3, _gravityConnected),
    [3, 0, 1]);
...
check('no wrap: empty cell at the seam keeps the component contiguous',
    _getConnectedSingleLayer(layerOf('--CuCuCu'), 1, _gravityConnected),
    [1, 2, 3]);
