...
- **Multi-distinct-quadrant targets still aren't solved within the cap — needs decomposition, not just a better heuristic.** Reachable, genuinely simple targets like `CuRuSuWu` (one quadrant cut from each of the four default starts, then stacked, ~15 ops) are not found within the state cap by A\*/BFS/IDA\*. The coverage heuristic (idea #1677, above) **massively improved the gradient** — A\* now descends to depth ~12 in <800 states, where the old similarity heuristic stalled at depth 4 indefinitely (a single-quadrant intermediate `Cu------` scored identically to the uniform start `CuCuCuCu`, so cutting toward the needed piece showed zero progress). But the targets still don't solve: the residual wall is **frontier width / state multiplicity near the goal** — thousands of near-equivalent partial assemblies plus accumulating cut-waste, the structural limit of forward best-first search over the multiset-of-shapes state space (the powerset blows up regardless of heuristic). The planned real fix is **constructive decomposition** (decompose target → layers → quadrants, build each piece, assemble via stack/swap), which sidesteps the search entirely — this is how every automated Shapez "make-anything-machine" works. Distinct from the memory issue above.
