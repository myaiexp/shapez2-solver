 ...
 */
function buildBackwardReachability(targetShapeCode, config, enabledOperations, maxDepth) {
...
async function shapeSolver(targetShapeCode, startingShapeCodes, enabledOperations, maxLayers, maxStatesPerLevel = Infinity, preventWaste, orientationSensitive, monolayerPainting, heuristicDivisor = 0.1, searchMethod = 'A*') {
    ...
    while (!cancelled) {
        ...
        while (stack.length > 0 && !cancelled) {
            ...
            if (totalIterations % 1000 === 0 || performance.now() - lastUpdate > 200) {
                self.postMessage({
                    type: 'status',
                    message: `IDA* | Iter:${iterationCount} | Threshold:${threshold} | Nodes:${nodesExplored} | Stack:${stack.length} | Total:${totalIterations}`
                });
                lastUpdate = performance.now();
        ...
        if (nextThreshold === Infinity || cancelled) {
            return cancelled ? null : { solutionPath: null, depth: null, statesExplored: totalIterations };
        }

        self.postMessage({
            type: 'status',
            message: `IDA* | Increasing threshold: ${threshold} → ${nextThreshold} | Total nodes: ${totalIterations}`
        });
    ...
    self.postMessage({ type: 'status', message: 'Bidirectional | Building backward reachability map...' });
    const backwardDepth = 4;
    const backwardMap = buildBackwardReachability(targetShapeCode, config, enabledOperations, backwardDepth);
    if (cancelled) return null;

    self.postMessage({
        type: 'status',
        message: `Bidirectional | Backward map: ${backwardMap.size} reachable shapes (depth ${backwardDepth})`
    });
    ...
    while (open.size() > 0 && !cancelled) {
        ...
        if (statesExplored % 500 === 0 || performance.now() - lastUpdate > 200) {
            self.postMessage({
                type: 'status',
                message: `Bidirectional | g=${g} | Open:${open.size()} | Explored:${statesExplored} | Total visited:${costSoFar.size} | Backward map:${backwardMap.size}`
            });
            lastUpdate = performance.now();
        }
    }
